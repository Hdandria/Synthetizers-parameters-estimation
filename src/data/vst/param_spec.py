...
class CategoricalParameter(Parameter):
    def __init__(
        ...
        if raw_values is not None:
            assert len(values) == len(raw_values), "values and raw_values must have the same length"
...
class NoteDurationParameter(Parameter):
    ...
    def sample(self) -> Tuple[float, float]:
        start, end = np.sort(np.random.uniform(0.0, self.max_note_duration_seconds, size=2)).tolist()
...
class ParamSpec:
    ...
    def encode(self, synth_param_dict: dict[str, float], note_param_dict: dict[str, float]) -> np.ndarray:
